...
Shrinkage is hardly new in finance. In 1956, Charles Stein published a famous paper called [*Inadmissibility of the usual estimator for the mean of a multivariate normal distribution*](https://projecteuclid.org/euclid.bsmsp/1200501656). In this work, he shows that, if the sum of the squares of the errors (SSE) is used as a loss function, then, for `$ n \geq 2 $`, the sample mean is not an optimal estimator and can be improved by exploring the trade-off among bias and variance. This improvement is done through shrinkage!
...
3. The shrinkage target (goes from 0 to 1 allowing for convex combinations between the highly structurerd and the non-structured estimators).
...
This code is almost identical to the one I had used in my previous post. If you want to see a detailed explanation on how it works, please, feel free to so by clicking [here](http://www.bernardo.codes/2018/08/27/how-many-stocks-in-the-s-p-500-do-follow-a-random-walk/). 
...
Working with unbalanced data (like the one above) challenges the estimation process. Let's add simple filter to exclude all the stock with less than `$ 4000 $` trading days (~ 15 years).
...
Despite the huge outlier presented in the estimation process around 2012, the amount of error for a period of less than a year is significant. For 3 months, the median error rate é about 80%. This value decreases sharply for one year and stabilizes after three years onwards. 
