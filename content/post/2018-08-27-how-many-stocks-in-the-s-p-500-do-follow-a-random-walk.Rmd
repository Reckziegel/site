...
So.. how do we stand? I will do my best to address this issue along the way.
...
`possibly()`, also from `purrr`, "guarantees" the main script will run. If any error occurs, we get a `NA`, but the code still runs. A safe bet for _loops_ with dozens of elements like this one. 
...
For 70% of our sample, the $ARIMA(0, 0, 0)$ won the day! On top of that, an additional 10% of total number of stocks are composed by $ARIMA(0, 0, 1)$, which also obeys the $E_t(y_{t + 1}) = E_t(y_{t})$ rule[^1]. The best point forecast for tomorrow is what we see in the screen today.  
