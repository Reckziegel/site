...
2. **List of countries covered by IMF**: this part of the code contains the countries provided by the IMF. The `mutate()` function in simply expand the `ODA/{COUNTRY}_{INDICATOR}` rule for each country in the data set.
...
4. **Error handler**: the `purrr` package, part of the `tidyverse`, offers interesting functions for manipulating data more safely. One of those function is `possibly()`, that always executes a function, if we ask for. If an error occurs, a custom message is printed. I chose `NA`.

5. **Data wrangling**: This task is divided in three. The first part tries to download data for all required countries (if no country is provided it will download the `indicator` for the whole data set). The second, excludes missing data, if there is any. The third, coerce the series in a time aware `tibble` in a nice tidy format.
...
The highlighted blue line shows the Brazilian debt. For most of the period analysed, Brazil's debt has been pushing LATAM average upwards. This will have important consequences as we approach the elections in October. There is a growthing number of specialists that believe the public spendig is the key for 

If you are interest in having a detailed picture on how the public spending should be managed I highly recomend you to read the World Bank report [A fair adjustment: efficiency and equity of public spending in Brazil](http://documents.worldbank.org/curated/en/643471520429223428/Volume-1-Overview).
